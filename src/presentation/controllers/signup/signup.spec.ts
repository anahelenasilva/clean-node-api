...
const makeSut = (): SutTypes => { // sytem under test
  ...
  const sut = new SignUpController(emailValidatorSutb, addAccountSutb)
  return {
    sut,
    emailValidatorSutb,
    addAccountSutb
...
describe('SignUp Controller', () => {
  ...
  test('Should call AddAccount with correct input', () => {
  ...
  test('Should return 500 if AddAccount throws exception', () => {
    const { sut, addAccountSutb } = makeSut()
    jest.spyOn(addAccountSutb, 'add').mockImplementationOnce(() => {
      throw new Error()
    })
  ...
  test('Should return 200 if valid input is provided', () => {
    const { sut } = makeSut()
